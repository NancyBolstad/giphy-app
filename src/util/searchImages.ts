...
async function searchImages(searchTerm: string, count: number = PAGE_SIZE, offset: number = 0) {
  let url = '';
  if (searchTerm === '') {
  ...
  url += `limit=${count}&offset=${offset}&rating=G&lang=en&api_key=${GIPHY_API_KEY}`;
